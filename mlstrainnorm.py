...
class ML:
    ...
    def process_training_data_7channels(self):
        nlabels = 8000
        with np.load(
            "/home/jass/Downloads/datasetstrain/Trainingstrainangles.npz"
        ) as data:
            # JA:seq[start:end:step]
            # Each has a different energy (4 total). All the same alpha
        ...
        if norm:
            for i in range(nlabels):
                train_examples_1[i, :, :] = train_examples_1[i, :, :] / np.max(
                    train_examples_1[i, :, :]
                )
                train_examples_2[i, :, :] = train_examples_2[i, :, :] / np.max(
                    train_examples_2[i, :, :]
                )
                train_examples_3[i, :, :] = train_examples_3[i, :, :] / np.max(
                    train_examples_3[i, :, :]
                )
                train_examples_4[i, :, :] = train_examples_4[i, :, :] / np.max(
                    train_examples_4[i, :, :]
                )
                train_examples_5[i, :, :] = train_examples_5[i, :, :] / np.max(
                    train_examples_5[i, :, :]
                )
                train_examples_6[i, :, :] = train_examples_6[i, :, :] / np.max(
                    train_examples_6[i, :, :]
                )
                train_examples_7[i, :, :] = train_examples_7[i, :, :] / np.max(
                    train_examples_7[i, :, :]
                )
        # print(train_labels.shape)
        train_dataset = tf.data.Dataset.from_tensor_slices(
        ...
        # train_dataset = train_dataset.map(lambda t: norm(t))
        ...
        nlabels = 2000
        with np.load(
            "/home/jass/Downloads/datasetstrain/Validationstrainangles.npz"
        ) as data:
        ...
        if norm:
            for i in range(nlabels):
                val_examples_1[i, :, :] = val_examples_1[i, :, :] / np.max(
                    val_examples_1[i, :, :]
                )
                val_examples_2[i, :, :] = val_examples_2[i, :, :] / np.max(
                    val_examples_2[i, :, :]
                )
                val_examples_3[i, :, :] = val_examples_3[i, :, :] / np.max(
                    val_examples_3[i, :, :]
                )
                val_examples_4[i, :, :] = val_examples_4[i, :, :] / np.max(
                    val_examples_4[i, :, :]
                )
                val_examples_5[i, :, :] = val_examples_5[i, :, :] / np.max(
                    val_examples_5[i, :, :]
                )
                val_examples_6[i, :, :] = val_examples_6[i, :, :] / np.max(
                    val_examples_6[i, :, :]
                )
                val_examples_7[i, :, :] = val_examples_7[i, :, :] / np.max(
                    val_examples_7[i, :, :]
                )
        val_dataset = tf.data.Dataset.from_tensor_slices(
            (
    ...
    def process_test_data_7channels(self) -> None:
        ...
        """
        nlabels = 2000
        with np.load("/home/jass/Downloads/datasetstrain/Teststrainangles.npz") as data:
            test_examples_1 = data["DataW"][0 : 3 * nlabels : 3]
            test_examples_2 = data["DataW"][1 : 3 * nlabels + 1 : 3]
            test_examples_3 = data["DataW"][2 : 3 * nlabels + 2 : 3]
            test_examples_4 = data["DataX"][0 : 4 * nlabels : 4]
            test_examples_5 = data["DataX"][1 : 4 * nlabels + 1 : 4]
            test_examples_6 = data["DataX"][2 : 4 * nlabels + 2 : 4]
            test_examples_7 = data["DataX"][3 : 4 * nlabels + 3 : 4]
        ...
        # nlabels = 7
        # with np.load("/home/jass/Downloads/datasetstrain/same padding/expdata/expdata.npz") as data:
        #     test_examples_1 = data["DataZ"][0 : 3 * nlabels : 3]
        #     test_examples_2 = data["DataZ"][1 : 3 * nlabels + 1 : 3]
        #     test_examples_3 = data["DataZ"][2 : 3 * nlabels + 2 : 3]
        #     test_examples_4 = data["DataX"][0 : 4 * nlabels : 4]
        ...
        for i in range(nlabels):
            ...
            for k in range(7):
                hists[k, :] = test_examples_{0}.format(k)[i, :, :].flatten()
                meantemp.append(sca[k, :].mean())
                stdtemp.append(sca[k, :].std())
            print(len(meantemp))
            ...
            textstr = "\n".join(
                (
                    r"$\mu=%.2f$" % (meantemp[0],),
                    r"$\mathrm{median}=%.2f, %.2f$" % (median, 2),
                    r"$\sigma=%.2f$" % (stdtemp[0],),
                )
            )
        ...
        if norm:
            for i in range(nlabels):
                test_examples_1[i, :, :] = test_examples_1[i, :, :] / np.max(
                    test_examples_1[i, :, :]
                )
                test_examples_2[i, :, :] = test_examples_2[i, :, :] / np.max(
                    test_examples_2[i, :, :]
                )
                test_examples_3[i, :, :] = test_examples_3[i, :, :] / np.max(
                    test_examples_3[i, :, :]
                )
                test_examples_4[i, :, :] = test_examples_4[i, :, :] / np.max(
                    test_examples_4[i, :, :]
                )
                test_examples_5[i, :, :] = test_examples_5[i, :, :] / np.max(
                    test_examples_5[i, :, :]
                )
                test_examples_6[i, :, :] = test_examples_6[i, :, :] / np.max(
                    test_examples_6[i, :, :]
                )
                test_examples_7[i, :, :] = test_examples_7[i, :, :] / np.max(
                    test_examples_7[i, :, :]
                )
        ...
        for i in range(nlabels):
            a = test_examples_1[i, :, :].flatten()
            plt.title("After norm max centered 0, goes from -1 to 1, 0 to 1")
            plt.hist(a, 50, facecolor="r")
            plt.show()
            plt.clf()
            a = test_examples_4[i, :, :].flatten()
            plt.hist(a, 50, facecolor="g")
            plt.show()
            plt.clf()
        for i in range(nlabels):
            m += test_examples_1[i, :, :].mean()
            s += test_examples_1[i, :, :].std()
