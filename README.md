...
- Time for creating datasets: The typical computational bottleneck we found lies in generating the datasets since the continuum model diagonalization for moir ́e systems can be computationally demanding. Producing 1000 samples with both $\mathcal{D}_{\bf{r}_{0}}\left(\omega\right)$ $\omega$ Dr0 (ω) and Dω0 (r) channels can take around 12 hours in a setup similar to computer B. Consequently, creating any dataset with 12000 samples from the main sections would take up to one week. In a parallelized environment such as the set of clusters from UIBK, we could
